	...
	SQL(Structured Query Language) : 데이터베이스에서 사용하는 데이터 처리 언어
		1. DDL( Data Definition Language ) 정의어		: 되돌리기(rollback) 불가능
			1. create : 데이터베이스/테이블 생성
                ...
                2. create table 테이블명( 필드명 타입 제약조건 , 필드명 타입 제약조건 , 필드명 타입 제약조건 );
		...
		2. DML( Date Manipulation Language ) 조작어*	: 되돌리기(rollback) 가능
			1.[C] insert : 레코드 삽입
				1. 모든 레코드 삽입 
		...
		- 제약조건
            ...
            3. default 
            4. primary key( pk필드명 )
            5. foreign key( fk필드명 ) references 참조할테이블명( 참조할 pk필드명 )
...
use test5;	-- 1. DB서버[서버PC]내 여러개의 데이터베이스가 존재하기 때문에 DB를 선택해야한다
