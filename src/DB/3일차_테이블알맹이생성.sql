...
-- 1. 데이터베이스 : 데이터가 모여있는 공간 = 테이블을 여러개 저장 할수 있는 공간

# 만일 'test3' 데이터베이스가 존재하면 삭제.
...
# 만일 'member' 테이블이 존재하면 삭제
drop table if exists member;

# 'member' 테이블 생성 create table 테이블명( 필드명 타입 )
create table member(
	mno int , 			-- 회원번호
    mid varchar(30) ,   -- 회원아이디 [ 최대 30글자 ]
    mphone char(13) ,	-- char고정길이 / 고정길이 쓰는 목적 : 필드의 저장할 문자의길이가 절대적으로 고정일때
						-- varchar가변길이 : 필드의 저장할 문자의길이가 명확하지 않을때
...
insert into member2( 회원번호1 ) values( 1 );		-- (특정 필드에 값 대입시 필드명 명시/작성) 오류 : 회원번호2필드가 not null 이기 때문에
...
# 5. foreign key(외래키)
	# 1. 다른 테이블의 기본키 와 연결/참조 된 정보 ( = 참조 )

# -------------- 회원제 게시판 : ( 회원들이 글을 작성해서 저장 ) ----------------- #
# 계층구조 : 회원(상위) 게시판(하위)
# 테이블 1개 : 서로다른 목적의 데이터들을 하나의 테이블로 작성시 이상현상(중복/불필요한정보)발생
# 테이블 2개(분해/정규화) : 동일한 목적의 데이터끼리 테이블을 생성해서 연결/관계(pk-fk) 이용한 관계형 데이터베이스 구축

# 5-1 회원 테이블 
	# 1.회원번호 / 2.아이디 / 3.비밀번호 / 4.이름		
    # 필드별 타입 선정 -> 제약조건 -> pk 선정( 식별키 : 중복X , 공백X , 하나의 레코드의 대표자 )
...
create table member5(
    ...
    mname varchar(20) default '익명'	,	-- 기본값이름이 '익명' 하지만 값 대입하면 대입값으로 저장됨.
...
create table board(
    ...
    mno_fk int ,						-- 참조테이블에 참조값(pk) 을 저장할 필드 선정
    foreign key( mno_fk ) references member5( mno )
    # foreign key( fk로사용할필드 ) references 참조할테이블( 참조할pk필드명 )
);